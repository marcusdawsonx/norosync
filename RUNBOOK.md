...
1. **`accounts: ["aa5f65-ed.myshopify.com"]` is MANDATORY on every `triplewhale`
   ...
   **The `facebook` connector has the same disease (found 05.08.2026):**
   unfiltered it returned ~5x true spend, summing multiple connected Meta ad
   accounts. Filter every `facebook` call to account_id **1047746717569933**
   ("Noro Arts") and cross-check daily spend against TW `attribution_spend`
   (must match to the cent from 18.07.2026 onward) before building.
...
4. **Klaviyo's reported figure is `attribution_revenue`** (total), not
   `attribution_new_customer_revenue`.
5. Shopify returns its date column as `day` and all values as strings.
...
| 4 | facebook | date, campaign, spend, impressions, clicks, actions_omni_purchase, action_values_omni_purchase — last 30d — **filtered to account_id 1047746717569933** (see rule 1) | `raw/meta_campaign_30d.json` |
